...
public class HelloWorldRestController {
	...
	public ResponseEntity<UserDetails> listAllUsers() {
		UserDetails users = userService.loadUserByUsername("");

		return new ResponseEntity<UserDetails>(users, HttpStatus.OK);
	}
	
	
	@RequestMapping(value = "/gigy/", method = RequestMethod.GET)
	public ResponseEntity<UserDetails> listAllUsers1() {
		UserDetails users = userService.loadUserByUsername("");
	 ...
	 * 
	 * if (userService.isUserExist(user)) { System.out.println("A User with name " +
